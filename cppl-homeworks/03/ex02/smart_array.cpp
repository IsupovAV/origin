...
smart_array::smart_array(const smart_array &source) : size_(source.size_), capacity_(source.capacity_) {
  data_ = new int[capacity_];
...
smart_array &smart_array::operator=(const smart_array &source) {
  if (this != &source) {
    delete[] data_;
    size_ = source.size_;
...
int smart_array::get_size() const { return size_; }
