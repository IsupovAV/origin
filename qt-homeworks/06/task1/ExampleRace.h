...
// Создаем класс в котором будем реализовывать код, выполняемый в нескольких
// потоках
...
public:
    ...
    // Передаем указатель на переменную которую будем инкрементировать, флак
    // включения мьютексов, и количесво итераций
...
// Именно так правильно использовать QThread, можно переопределить класс run, но
// разработчики Qt рекомендуют делать именно так.
...
public:
    Controller(QMutex *m) : mut(m) {
        ...
        connect(
            &workerThread, &QThread::finished, race,
        ...
        connect(race, &ExampleRace::sig_Finish, this,
                &Controller::sig_WorkFinish); // Пробрасываем сигнал для отображения
            // в GUI
